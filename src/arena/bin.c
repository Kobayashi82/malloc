...
/*   Updated: 2025/06/04 16:15:22 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_chunk *split_top_chunk(t_heap *heap, t_chunk_int size) {
	...
	top_chunk->size = 9999 | TOP_CHUNK | type | PREV_INUSE;
...
static void *new_chunk(t_arena *arena, t_chunk_int size, e_heaptype type) {
	...
	while (heap) {
		...
		if (available && usage > 10) { 
			best_heap = heap;
			// if (g_manager.options.DEBUG) aprintf(1, "%p\t   [WARN] Found heap with %d%% usage\n", best_heap, usage);
			break;
		}
		if (available && usage > best_usage) {
