...
/*   Updated: 2025/06/03 21:54:49 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned char get_zonetype(size_t size) {
	if		(size <= TINY_USER)		return (TINY);
	else if (size <= SMALL_USER)	return (SMALL);
...
unsigned char get_freelist_index(size_t size) {
	...
	if		(size == 0)				size = 1;
	if 		(size <= TINY_USER)		{ max = TINY_USER;	base = 1; }
	else if (size <= SMALL_USER)	{ max = SMALL_USER;	base = TINY_USER + 1; }
...
void print_freelist_ranges() {
	int tiny_base = 1;
	int tiny_range = TINY_USER - tiny_base + 1;
	...
	for (int i = 0; i < FREELIST_SIZE; ++i) {
		...
		if (max > TINY_USER) max = TINY_USER;
		printf("  [%2d] %4d - %4d bytes\n", i, min, max);
	}

	int small_base = TINY_USER + 1;
	int small_range = SMALL_USER - small_base + 1;
	...
	for (int i = 0; i < FREELIST_SIZE; ++i) {
		...
		if (max > SMALL_USER) max = SMALL_USER;
